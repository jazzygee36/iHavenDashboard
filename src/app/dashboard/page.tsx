...
const Dashboard = () => {
  ...
  const courseCount = courses.filter((course: any) => course).length;
  const studentCount = users.filter((user: any) => user).length;
  const instructorCount = new Set(
    courses
      .filter((course: any) => course.instructorsName)
      .map((course: any) => course.instructorsName)
  ).size;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {CardDetails.map((item, index) => (
          ...
          >
            ...
            <div
              className={`absolute -top-4 -right-4 h-20 w-20 opacity-10 rounded-full ${item.color}`}
            />
